...
router.get("/", async (req, res) => {
  ...
  const xml = xmlBuilder.buildObject({ alquileres: results });
...
router.post("/", async (req, res) => {
  ...
  if (!result) {
    ...
    res.status(200).json({ message: 'Alquiler creado', result });
  }
});
...
router.get("/editar/:id", async (req, res) => {
  ...
  let vivienda = await dbConnect.collection(COLLECTION).findOne(query);
  if (!vivienda) {
...
router.put("/:id", async (req, res) => {
  ...
  if (!result.value) {
    ...
    // res.redirect("/alquiler");
    res.status(200).json({ message: 'Alquiler actualizado', result });
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  if (result.deletedCount === 0) {
    ...
    // res.redirect("/alquiler");
    res.status(200).json({ message: 'Alquiler eliminado', result });
  }
});
...
router.get("/barrio/:barrio", async (req, res) => {
  ...
  res.status(200).json({ message: 'Alquileres encontrados', results });
  // res.render("alquiler", { title: "ApViMad", viviendas: results });
});
